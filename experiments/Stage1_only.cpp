...
#pragma omp parallel for
    ...
    cout << "\n>>> Starting Experiment: Dynamic Alpha (Linear Curve) <<<" << endl;


    size_t ef_search = 200;


    std::vector<float> A_list = {0.0f, 10.0f, 50.0f, 100.0f, 200.0f, 300.0f, 500.0f, 1000.0f, 2000.0f, 5000.0f};


    cout << "---------------------------------------------------------------" << endl;
    cout << std::left << std::setw(10) << "Alpha(A)"
    ...
    for (float A : A_list) {
        ...
        for (int i = 0; i < num_test; ++i) {
            ...
            std::vector<int> res = index.SearchDynamicAlpha(data_query + i * dim_query, K, ef_search, A, 1);
