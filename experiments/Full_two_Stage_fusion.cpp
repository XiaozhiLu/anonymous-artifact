...
int main() {
    ...
    cout << "\n>>> Starting Fusion Experiment: Scheme 1 (Source) + Scheme 2 (Refine) <<<" << endl;

    float epsilon = 0.2f;


    float fixed_alpha_search = 200.0f;


    std::vector<float> beta_list = {0.2f, 0.5f, 0.8f, 1.0f, 2.0f, 5.0f, 10.0f, 20.0f, 50.0f, 100.0f};

    cout << "Fixed Alpha_Search (A) = " << fixed_alpha_search << endl;
    ...
    for (float b : beta_list) {
        ...
        for (int i = 0; i < num_test; ++i) {
            ...
            std::vector<int> res = index.SearchIterative(data_query + i * dim_query, K, epsilon, b, fixed_alpha_search);
